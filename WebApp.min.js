...
else{Result="<b>Unknown Content-Type!<\/b>";};return Result;};this.DoRun=function(){this.FDatabase=$lt2.$create("Create$1",[this]);this.GetContent("\/NaR");$lp2().InitHistory(2,"");$lp2().RegisterRoute("*",rtl.createCallback(this,"HandleRoute"),false);this.AfterInit(false);};this.GetContent=function(APath){var Result="";this.FDatabase.SetFilter("Path="+$lp(APath,"'"));if(this.FDatabase.GetDataSet().FEOF){Result=""}
else Result=this.RenderContent(this.FDatabase.GetInts("ContentType"));return Result;};this.AfterInit=function(RealRun){if(!RealRun)return;if($lp2().RouteFromURL()==="")$lp2().Push("\/index");};});this.WEBSITE_TITLE="kveroneau.github.io";this.DATE_FORMAT='dddd mmmm d, yyyy "at" hh:nn';this.Application=null;$mod.$main=function(){$mod.Application=$lt.$create("Create$1",[null]);$mod.Application.Initialize();$mod.Application.Run();};});